#!/usr/bin/env python
"""Import data - a "list of lists" from Pickle file, Caclulated difference in counters
between each poll and create a chart"""
#### import statements
import pickle
import pygal
...
def lister(some_data, data_position):
	"""Extract a record from each row, or list in the list variable"""
    output = []
...
def calc_diffs(a_list_raw):
	"""For each value, subract it from the previous value. If the counter rolled over 
	between polls, add the max counter value and then subract"""
